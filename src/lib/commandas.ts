...
export async function syncAppointmentComanda(db: DbClient, input: AppointmentComandaInput) {
  ...
  // Commission entries are generated by triggers and reference commanda_items
  // with ON DELETE RESTRICT. Remove the still-unsettled ones before rebuilding
  // the items, otherwise the delete below fails with a FK violation.
