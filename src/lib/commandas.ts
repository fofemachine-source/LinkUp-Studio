...
export async function syncAppointmentComanda(db: DbClient, input: AppointmentComandaInput) {
  ...
  if (!commanda) {
    ...
    const { data: created, error } = await db
      ...
      .insert({
        ...
        number,
        // Keep the comanda open until its items are rebuilt. Closing here fires
        // the commission trigger against the old items and makes them impossible
        // to delete because commission_entries references them with RESTRICT.
        status: "open",
        closed_at: null,
        payment_method: null,
        subtotal: total,
    ...
    const { error } = await db
      ...
      .update({
        ...
        client_id: input.clientId ?? null,
        client_name: input.clientName,
        // Rebuild the item snapshot before closing and generating commissions.
        status: "open",
        closed_at: null,
        payment_method: null,
        subtotal: total,
  ...
  // Recover commandas affected by the old close-before-rebuild sequence. A
  // pending commission normally already has a payable movement, so payable_id
  // must not be used as a deletion filter here. Settled/paid entries remain
  // protected and are never removed by this repair path.
  const { error: commissionDeleteError } = await db
  ...
  if (commissionDeleteError) throw commissionDeleteError;
  ...
  // Only now may the close trigger generate commissions: every generated row
  // will reference the final item snapshot that remains stored on the comanda.
