-- Allow first access for pre-registered booking customers without a manual code
-- when the CPF exists, no booking password/account exists yet, and the provided
-- WhatsApp matches the salon/subscription record. CPF alone is not enough.

create or replace function private.normalize_booking_whatsapp(p_value text)
returns text
language sql
immutable
set search_path = ''
as $$
  select case
    when length(regexp_replace(coalesce(p_value, ''), '[^0-9]', '', 'g')) in (12, 13)
      and left(regexp_replace(coalesce(p_value, ''), '[^0-9]', '', 'g'), 2) = '55'
      then substring(regexp_replace(coalesce(p_value, ''), '[^0-9]', '', 'g') from 3)
    else regexp_replace(coalesce(p_value, ''), '[^0-9]', '', 'g')
  end;
$$;

revoke all on function private.normalize_booking_whatsapp(text)
...
declare
  ...
  v_whatsapp_matches_existing_customer boolean := false;
  v_has_activation_code boolean := false;
begin
  if not exists (
    select 1
  ...
  v_has_activation_code := p_activation_code is not null and btrim(p_activation_code) <> '';
  ...
  if v_has_existing_account then
    ...
    if not v_has_activation_code then
      raise exception 'CUSTOMER_ACCOUNT_EXISTS' using errcode = 'P0001';
    end if;
  ...
  if found then
    select exists (
      ...
      where subscription.tenant_id = p_tenant_id
        and (
          subscription.client_id = v_client.id
          or regexp_replace(coalesce(subscription.cpf, ''), '[^0-9]', '', 'g') = p_cpf
        )
    )
    into v_has_subscription;

    v_whatsapp_matches_existing_customer :=
      private.normalize_booking_whatsapp(v_client.whatsapp) = p_whatsapp
      or exists (
        select 1
        from public.client_subscriptions as subscription
        where subscription.tenant_id = p_tenant_id
          ...
          and private.normalize_booking_whatsapp(subscription.whatsapp) = p_whatsapp
      );

    if v_has_subscription and not v_whatsapp_matches_existing_customer then
      if not v_has_activation_code then
    ...
    if v_has_subscription then
      v_whatsapp_matches_existing_customer := exists (
        select 1
        from public.client_subscriptions as subscription
        where subscription.tenant_id = p_tenant_id
          and regexp_replace(coalesce(subscription.cpf, ''), '[^0-9]', '', 'g') = p_cpf
      ...
      if not v_whatsapp_matches_existing_customer then
        raise exception 'EXISTING_CUSTOMER_REQUIRES_ACTIVATION' using errcode = 'P0001';
      end if;
    end if;
...
-- The access-code fallback must be callable by logged-in salon staff.
